...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const azureConfigPanel = document.querySelector('.azure-config-panel');
    const saveAzureConfigBtn = document.getElementById('save-azure-config');
...
AGI pode pensar e resolver muitos problemas como uma pessoa.`
    ...
    // Função para carregar configuração do Azure
    function loadAzureConfig() {
        const azureKey = localStorage.getItem('azureKey');
        const azureRegion = localStorage.getItem('azureRegion') || 'eastus';
        ...
        if (azureKey) {
            ...
            statusMessage.textContent = 'Configure sua chave do Azure para habilitar o download.';
        }
    }

    // Função para salvar configuração do Azure
    function saveAzureConfig() {
        const key = azureKeyInput.value.trim();
        const region = azureRegionInput.value.trim();
        
        if (!key) {
            alert('Por favor, insira uma chave de API válida do Azure.');
            return;
        }
        ...
        statusMessage.textContent = 'Configuração do Azure salva com sucesso!';
        downloadBtn.disabled = false;
    }

    // Função para atualizar a voz com base no idioma selecionado
    function updateVoiceBasedOnLanguage() {
        const selectedLanguage = languageSelect.value;
        const recommendedVoice = voiceMapping[selectedLanguage];
        
        // Atualizar a seleção de voz
        if (recommendedVoice) {
            voiceSelect.value = recommendedVoice;
        }
    }
    ...
    function handleDownloadAudio() {
        ...
        // Verificar se há configuração do Azure
        const azureKey = localStorage.getItem('azureKey');
        if (!azureKey) {
            statusMessage.textContent = 'Configure sua chave do Azure para habilitar o download.';
            azureConfigPanel.style.display = 'block';
            return;
        }
